export default {
  lang: {
    ru: "Русский",
    ua: "Украинский",
    en: "Английский",
  },
  title: {
    first: "Оставайся на связи с семьёй и друзьями",
    second: "с интернетом или без него",
  },
  connected: "Ваш текущий сервер  <b>{name}</b>",
  server: "Дугие сервера:",

  subtitle:
    "Подорка децентрализованных сервисов, которые вы сможете использовать когда Telegram, Whatsapp, Facebook будут более недоступны",
  alert: {
    title: "Не жди. Маcштабируй!",
    descr:
      "Создавайте свои собственные сети используя код. Присоединяйтесь к технической группе для обсуждения сотрудничества, новых услуг и волонтерства.",
    buttons: {
      repo: "Репозиторий",
      servers: "Matrix сервер",
    },
  },
  cities: {
    moscow: "Москва",
    piter: "Санкт-Петербург",
    samara: "Самара",
    krasnodar: "Краснодар",
    ekaterinburg: "Екатеринбург",
    novosibirsk: "Новосибирск",
    vladivostok: "Владивосток",
    khabarovsk: "Хабаровск",
  },

  footer: {
    initiative: `Инициатива — <a target="_blank" href="https://equalit.ie/">eQualit.ie</a>`,
    license: `Под лицензией <a  href="https://github.com/censorship-no/dcomms"> GNU General Public License v3.0</a>`,
  },
  rubric: {
    messaging: "Переписка",
    browsing: "Просмотр сайтов",
    //    filesharing: "Обмен файлами",
  },
  adressCopy: "Адрес скопирован",

  items: {
    desktopApp: "Подключиться с компьютера:",
    localDownload: "локальное скачивание",
    instruction: "Инструкция:",
    downloads: "Скачать:",
    notAvail: "Недоступен",
    mobileApp: "Приложение для телефона:",
  },
  copyUrl: "CopyURL",
};
